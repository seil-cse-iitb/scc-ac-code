...
//const uint64_t rAddress = 0x00001E5000LL; // Address for classroom 205
const uint64_t rAddress = 0x00001E5205LL; // Address for classroom 205
...
// Data structure to store the data received over RF
struct data_rx
{
  char ac_number = '\0';  // # of AC to control (AC number can be 'A' or 'ac_id')
  char ac_state = '\0';   // Turn it ON or OFF  (AC state is 'O' for on and 'F' for off)
} data;
...
void loop()
{
  ...
  {
    ser_ac_state = Serial.read();
    serRead = true;
  }

  if (serRead || received)
  {
    if (data.ac_number == ac_id || data.ac_number == 'A')
    {
      radio.stopListening();
      if (data.ac_state == 'O')  turn_on_ac();
      else if (data.ac_state == 'F') turn_off_ac();
